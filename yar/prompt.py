...
1.  **Entity Extraction & Output:**
    ...
    *   **Branding-only mentions:** Do NOT extract organizations that appear only in logos, headers, footers, copyright notices, or slide branding (e.g., `[Company Logo]`) unless the text states a concrete relationship for that organization.
    ...
    *   **Abbreviations and Aliases:**
        *   When an entity appears as both an abbreviation and full name (e.g., "NHA" and "National Health Agency"), extract ONLY the most commonly used form, typically the abbreviation for well-known entities.
        *   Include the alternative form in the description (e.g., "NHA (National Health Agency) is the regulatory agency...")
...
2.  **Relationship Extraction & Output:**
    ...
    *   **What NOT to Extract:**
        ...
        *   For "X on Y" event names, keep the event and object separate when the text treats Y as the reviewed/read/sent document or topic. Example: `Management Teleconference` reviewed `Review Pre-reads`, not one fused entity named `Management Teleconference on Review Pre-reads`.
        *   For meeting agreement/postponement bullets, extract the agreed action or decision as the target entity when it is explicit. Example: `Steering Committee Meeting` agreed on `Submission Delay Decision`.
        *   For wording-alignment bullets tied to a prior named meeting, connect the alignment event to that meeting (e.g., `Slide Wording Alignment` followed `Steering Committee Meeting`) unless the slide deck itself is emitted as an entity.
...
9.  **Type Guidance:** when assigning `entity_type`, follow these per-type definitions consistently across the extraction (this prevents the same concept being labeled with different types in different chunks, which blocks downstream entity merging):
    ...
    *   **Event:** named sessions, reviews, audits, meetings, project events, milestones (e.g., "Quality Review 2024", "Regulatory Review Meeting").
    *   **Document:** named documents, templates, reports, plans, charts, pages, links, resource pages (e.g., "Action Plan", "IND Dossier", "SharePoint Resource Page").
    *   **Product:** named drug products, components, materials, intermediates when treated as items (e.g., "Product A", "Drug Product 50mg Tablet").
...
11. **Recommended Extraction Patterns:**
    *   **Review/audit tables:** treat the named site/facility as Organization; the named review/audit title as Event; action plans, templates, indexes, pages, and links as Document. Link the organization/site to the action plan with `outlined`, `accepted`, `responsible for`, or `addresses`.
...
<Input Text>
```
Contoso Health announced that the NHA has approved Oncora (lumabex) for the treatment of alpha-cell carcinoma. The approval was based on results from the ALPHA-024 clinical trial, which demonstrated a 40% reduction in disease progression. Dr. Morgan Lee, President of Contoso Research Laboratories, stated that this approval represents a significant advancement in immunotherapy. Oncora works by blocking PX-1, a protein that helps tumor cells evade the immune system.
```

<Output>
entity{tuple_delimiter}Contoso Health{tuple_delimiter}organization{tuple_delimiter}Contoso Health is a pharmaceutical company that manufactures Oncora and announced the NHA approval.
entity{tuple_delimiter}NHA{tuple_delimiter}organization{tuple_delimiter}The NHA (National Health Agency) is the regulatory agency that approved Oncora for alpha-cell carcinoma treatment.
entity{tuple_delimiter}Oncora{tuple_delimiter}artifact{tuple_delimiter}Oncora (lumabex) is an immunotherapy drug approved for treating alpha-cell carcinoma by blocking PX-1.
entity{tuple_delimiter}Alpha-Cell Carcinoma{tuple_delimiter}concept{tuple_delimiter}Alpha-cell carcinoma is the disease condition that Oncora is approved to treat.
entity{tuple_delimiter}ALPHA-024{tuple_delimiter}event{tuple_delimiter}ALPHA-024 is the clinical trial that demonstrated Oncora's efficacy with a 40% reduction in disease progression.
entity{tuple_delimiter}Morgan Lee{tuple_delimiter}person{tuple_delimiter}Dr. Morgan Lee is the President of Contoso Research Laboratories who commented on the approval.
entity{tuple_delimiter}PX-1{tuple_delimiter}concept{tuple_delimiter}PX-1 is a protein that Oncora blocks to help the immune system fight tumor cells.
relation{tuple_delimiter}Contoso Health{tuple_delimiter}Oncora{tuple_delimiter}manufactures{tuple_delimiter}Contoso Health is the pharmaceutical company that manufactures Oncora.
relation{tuple_delimiter}NHA{tuple_delimiter}Oncora{tuple_delimiter}approved{tuple_delimiter}The NHA approved Oncora for the treatment of alpha-cell carcinoma.
relation{tuple_delimiter}Oncora{tuple_delimiter}Alpha-Cell Carcinoma{tuple_delimiter}treats{tuple_delimiter}Oncora is approved as a treatment for alpha-cell carcinoma.
relation{tuple_delimiter}Morgan Lee{tuple_delimiter}Contoso Health{tuple_delimiter}leads{tuple_delimiter}Dr. Morgan Lee is the President of Contoso Research Laboratories.
relation{tuple_delimiter}ALPHA-024{tuple_delimiter}Oncora{tuple_delimiter}evaluated{tuple_delimiter}ALPHA-024 evaluated Oncora and demonstrated reduced disease progression.
relation{tuple_delimiter}Oncora{tuple_delimiter}PX-1{tuple_delimiter}blocks{tuple_delimiter}Oncora blocks PX-1 to help the immune system fight tumor cells.
...
<Input Text>
```
# Supplier quality escalation – lessons learned
D. Lee, New Product Launch lessons sharing, 27th Feb 2019
[Company Logo]

# 3mL closure component issue
Delivery from supplier Northwind Components showed fibers and metal particles.
Closure component batches from Northwind Components could not be released.
Closure components could not be used for manufacturing of primary stability batches.

# Sequence of events
* 23rd Nov => Quality Review meeting
    * Assessment of intermediate/final closure component
* 28th/29th Nov => Steering Committee meeting
    * Agreement on postponement of submission
* 29th Nov => wording on governance slides aligned after Steering Committee meeting ended
* 30th Nov => Midday review pre-reads sent out
* 3rd Dec => Management teleconference on review pre-reads
    * Participants: Rivera, Okafor
```

<Output>
entity{tuple_delimiter}Supplier Quality Escalation{tuple_delimiter}document{tuple_delimiter}Supplier Quality Escalation is the 27th Feb 2019 lessons-learned document for new product launch sharing.
entity{tuple_delimiter}D. Lee{tuple_delimiter}person{tuple_delimiter}D. Lee is the author of the Supplier Quality Escalation document.
entity{tuple_delimiter}Northwind Components{tuple_delimiter}organization{tuple_delimiter}Northwind Components is the supplier whose closure component batches showed fibers and metal particles.
entity{tuple_delimiter}3mL Closure Component{tuple_delimiter}artifact{tuple_delimiter}3mL Closure Component is the affected component whose Northwind Components batches could not be released or used for primary stability batch manufacturing.
entity{tuple_delimiter}Quality Review Meeting{tuple_delimiter}event{tuple_delimiter}Quality Review Meeting is the 23rd Nov event that assessed the intermediate/final closure component.
entity{tuple_delimiter}Steering Committee Meeting{tuple_delimiter}event{tuple_delimiter}Steering Committee Meeting is the 28th/29th Nov event where postponement of submission was agreed.
entity{tuple_delimiter}Submission Delay Decision{tuple_delimiter}event{tuple_delimiter}Submission Delay Decision is the agreed postponement of submission.
entity{tuple_delimiter}Slide Wording Alignment{tuple_delimiter}event{tuple_delimiter}Slide Wording Alignment is the 29th Nov event where wording on governance slides was aligned after the meeting.
entity{tuple_delimiter}Review Pre-reads{tuple_delimiter}document{tuple_delimiter}Review Pre-reads are the documents sent out on 30th Nov for the Management Teleconference.
entity{tuple_delimiter}Management Teleconference{tuple_delimiter}event{tuple_delimiter}Management Teleconference is the 3rd Dec teleconference that reviewed pre-reads.
entity{tuple_delimiter}Rivera{tuple_delimiter}person{tuple_delimiter}Rivera is a participant in the Management Teleconference.
entity{tuple_delimiter}Okafor{tuple_delimiter}person{tuple_delimiter}Okafor is a participant in the Management Teleconference.
relation{tuple_delimiter}D. Lee{tuple_delimiter}Supplier Quality Escalation{tuple_delimiter}authored{tuple_delimiter}D. Lee authored the Supplier Quality Escalation document dated 27th Feb 2019.
relation{tuple_delimiter}Northwind Components{tuple_delimiter}3mL Closure Component{tuple_delimiter}supplied contaminated batches{tuple_delimiter}Northwind Components supplied closure component batches that showed fibers and metal particles.
relation{tuple_delimiter}Quality Review Meeting{tuple_delimiter}3mL Closure Component{tuple_delimiter}assessed{tuple_delimiter}The Quality Review Meeting assessed the intermediate/final closure component.
relation{tuple_delimiter}Steering Committee Meeting{tuple_delimiter}Submission Delay Decision{tuple_delimiter}agreed on{tuple_delimiter}The Steering Committee Meeting agreed on postponement of submission.
relation{tuple_delimiter}Slide Wording Alignment{tuple_delimiter}Steering Committee Meeting{tuple_delimiter}followed{tuple_delimiter}Slide Wording Alignment followed the Steering Committee Meeting.
relation{tuple_delimiter}Review Pre-reads{tuple_delimiter}Management Teleconference{tuple_delimiter}sent for{tuple_delimiter}Review Pre-reads were sent out for the Management Teleconference.
relation{tuple_delimiter}Management Teleconference{tuple_delimiter}Review Pre-reads{tuple_delimiter}reviewed{tuple_delimiter}Management Teleconference reviewed Review Pre-reads on 3rd Dec.
relation{tuple_delimiter}Management Teleconference{tuple_delimiter}Rivera{tuple_delimiter}included participant{tuple_delimiter}Rivera participated in the Management Teleconference.
relation{tuple_delimiter}Management Teleconference{tuple_delimiter}Okafor{tuple_delimiter}included participant{tuple_delimiter}Okafor participated in the Management Teleconference.
{completion_delimiter}
...
```
# Executive summary
* The use of closed transfer adapters can pose risks to product quality and accurate dosing.
* The transfer-adapter strategy recommendations mitigate risks to product quality and accurate dosing.
```

<Output>
entity{tuple_delimiter}Closed Transfer Adapter{tuple_delimiter}artifact{tuple_delimiter}Closed Transfer Adapter is a transfer device whose use can pose risks to product quality and accurate dosing.
entity{tuple_delimiter}Product Quality{tuple_delimiter}concept{tuple_delimiter}Product Quality is the affected outcome that closed transfer adapter use can put at risk and that strategy recommendations mitigate.
entity{tuple_delimiter}Accurate Dosing{tuple_delimiter}concept{tuple_delimiter}Accurate Dosing is the affected outcome that closed transfer adapter use can put at risk and that strategy recommendations mitigate.
entity{tuple_delimiter}Transfer Adapter Strategy Recommendations{tuple_delimiter}document{tuple_delimiter}Transfer Adapter Strategy Recommendations mitigate risks to product quality and accurate dosing.
relation{tuple_delimiter}Closed Transfer Adapter{tuple_delimiter}Product Quality{tuple_delimiter}poses risk to{tuple_delimiter}The use of closed transfer adapters can pose risks to product quality.
relation{tuple_delimiter}Closed Transfer Adapter{tuple_delimiter}Accurate Dosing{tuple_delimiter}poses risk to{tuple_delimiter}The use of closed transfer adapters can pose risks to accurate dosing.
relation{tuple_delimiter}Transfer Adapter Strategy Recommendations{tuple_delimiter}Product Quality{tuple_delimiter}mitigates risk to{tuple_delimiter}Transfer-adapter strategy recommendations mitigate risks to product quality.
relation{tuple_delimiter}Transfer Adapter Strategy Recommendations{tuple_delimiter}Accurate Dosing{tuple_delimiter}mitigates risk to{tuple_delimiter}Transfer-adapter strategy recommendations mitigate risks to accurate dosing.
{completion_delimiter}
...
- Document Chunks: evidence, quotes, supporting detail. If a chunk includes `evidence_spans`, treat those spans as the first-pass support for the answer and verify each claim against them before using broader chunk text.
  - For lessons-learned, best-practice, failure-analysis, or role questions, include the relevant exact phrase from an `evidence_spans` item before paraphrasing it.
- Knowledge-graph relationships alone do not stand as answers; they may provide indirect support when corroborated by document chunks. Cite the chunk that contains the supporting evidence, not the relationship itself.
  - Do not include raw knowledge-graph triples, arrow notation (`A --rel--> B`), JSON objects, or internal relationship annotations in the final answer; translate only directly supported facts into prose and cite the supporting document chunk.
- Never use world knowledge or training knowledge. Every factual claim must be supportable by the retrieved context.
- Every sentence in the answer MUST correspond directly to a contiguous span in a retrieved chunk or `evidence_spans`. Structural organizer claims such as "X has three steps" are allowed only when the same organizing fact appears in the context.
- When citing multiple chunks, keep each chunk's contribution as a separate cited claim; do not blend facts from different chunks into one synthesized sentence.
...
- Conflicting/mixed-topic context: ignore unrelated portions, do not blend.
  If timelines, dates, approvals, submissions, roles, or impacts belong to different products, projects, studies, people, or documents, keep those facts separated. Never transfer a milestone/date/role/impact from one named subject to another unless the same retrieved chunk explicitly links both named subjects to that fact.
- When a document has both metadata/header chunks and substantive sections (best practices, lessons, action plans, numbered steps), answer process/lesson/action questions from the substantive sections, not from the metadata header alone.
...
- Prefer close paraphrase or exact source wording for reasons, objectives, roles, and impacts. Do not add interpretive glosses, causal bridges, or high-level rationales that are not stated in the context.
- When structured tables or labeled fields appear in context, extract every field directly relevant to the question (for example Objective, Target potential users, Context for LL, Sponsor, Status, Name the Best Practice). Keep labels when they prevent ambiguity.
- When structured tables have distinct rows or columns such as Name, Context, Objective, Status, Owner, or Target, keep each label bound to its own value. Do not merge a context/objective value into a name field, and do not reattach a subordinate clause or modifier to a different noun than the one it modifies in the source.
- Preserve source abbreviations and acronyms exactly unless the retrieved context gives their expansion; do not guess acronym expansions.
...
- Cite each fact with the reference_id of the source document chunk that contains that exact fact. If one sentence needs facts from different sources, split it into separate cited sentences unless the combined sentence is directly supported by all cited sources.
...
- For why/significance/importance/impact questions, do not require the source to contain the exact words "why" or "significance". If the context states a milestone, approval, delay, risk, dependency, commitment, cost, or downstream impact for the queried entity, answer with those supported facts first and mention only missing subparts afterward.
- If a named entity appears as one item in a parallel list under a heading, bullet, table row, or slash/comma-separated stakeholder list, treat the list-level statement as applying to that named item unless the context explicitly distinguishes the listed items. Do not refuse solely because the named item is not the grammatical subject of its own sentence.
...
- Mandatory-prefix rule for (a)-(d) when Step 4 fires: the FIRST sentence of your answer MUST be `The retrieved context does not describe the <asked thing> of <queried entity>.`, filling `<asked thing>` and `<queried entity>` from the question. After that sentence, STOP. You may optionally add ONE additional sentence naming what the context DOES cover (e.g. `The context covers the qualification status and acceptance criteria of <entity>.`) but you may NOT continue with `However, ...`, `Additionally, ...`, `It is known that ...`, or any sentence introducing a fact about the asked thing. The escape hatch where a model "found a verbatim quote" is REMOVED for this rule — if you genuinely find a verbatim mechanism / target / origin statement in chunks, you would not be in Step 4. You may not begin the answer with a description, a paraphrase, or an entity definition. Examples of forbidden openings (each is a direct violation of this rule): "X targets <anything>", "X is a method that <anything>", "Y uses <anything>", "The mechanism of X involves <anything>", "The detection principle is <anything>", "The molecular target of X is <anything>", "The origins of X trace to <anything>". If you find yourself starting any sentence with such a construction without first writing the refusal sentence above, you have violated the contract — restart with the refusal sentence. Once the refusal sentence has been written, do not contradict it with subsequent training-data content.
...
- List/count/enumeration questions: include all supported items, not first few; start by restating the requested subject using the user's wording. If context contains a heading, numbered list, bullet list, or table matching the requested section/topic, preserve source numbering/rows when present and include each visible item explicitly. Do not refuse only because the source uses bullets/rows rather than the word "items". Include only items the context explicitly identifies as members of the requested category. List answers in single paragraph: keep each supported item explicit and separate them cleanly with semicolons; no narrative blending.
...
- Factual lookup grounded in a specific table/section:
  - Identify the exact entity / action / step / section name referenced by the question.
...
- Say "insufficient information" only when context has nothing relevant. If any retrieved chunk directly matches the requested section, list, row, or one side of a comparison, answer the supported part first and mention only missing subparts afterward.
...
2. **low_level_keywords** (1-4): specific entities EXPLICIT in query:
   - Companies/orgs: "OpenAI", "Microsoft", "NHA", "Tesla"
   - People: "Elon Musk", "Sam Altman", "Tim Cook"
   - Products/tech: "GPT-4", "iPhone", "Azure", "Oncora"
...
2. **Preserve Exact Names**: low-level keywords keep entity names verbatim (no "Oncora" -> "drug").
...
Query: "What medicines did the NHA approve for diabetes in 2024?"

Output:
{
  "high_level_keywords": ["drug approval", "regulatory approval", "diabetes treatment"],
  "low_level_keywords": ["NHA", "diabetes", "2024"]
}
...
""",
    """Example 9 (Manufacturing strategy query):

Query: "What is the access-control device strategy in Facility B?"

Output:
{
  "high_level_keywords": ["device strategy", "manufacturing process", "access control"],
  "low_level_keywords": ["access-control device", "Facility B"]
}

""",
    """Example 10 (Dates/milestones/timeline query):

Query: "What are the dates or milestones mentioned during the platform migration period?"

Output:
{
  "high_level_keywords": ["history", "background", "chronology", "timeline"],
  "low_level_keywords": ["platform migration"]
}

""",
    """Example 11 (Temporal/when query):

Query: "When did the NHA approve Product A and what were the subsequent submission events?"

Output:
{
  "high_level_keywords": ["history", "regulatory timeline", "approval chronology"],
  "low_level_keywords": ["NHA", "Product A", "approval", "submission"]
}
...
- Abbreviations: "NHA" = "National Health Agency"
...
- Initial-prefixed names: "P. Rivera" = "Rivera", "D. Lee" = "Lee" when context confirms the same person
