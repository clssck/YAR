...
from yar.kg.shared_storage import finalize_share_data
from yar.utils import get_env_value, setup_logger
...
errorlog = os.getenv('ERROR_LOG', log_file_path)  # Default write to yar.log
accesslog = os.getenv('ACCESS_LOG', log_file_path)  # Default write to yar.log
...
def post_fork(server, worker):
    ...
    setup_logger('yar', log_level, add_filter=True, log_file_path=log_file_path)

    # Set up yar submodule loggers
    for name in logging.root.manager.loggerDict:
        if name.startswith('yar.'):
