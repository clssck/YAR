# YAR Server and WebUI

The YAR Server is designed to provide a Web UI and API support. The Web UI facilitates document indexing, knowledge graph exploration, and a simple RAG query interface. YAR Server also provides an Ollama-compatible interface, aiming to emulate YAR as an Ollama chat model. This allows AI chat bots, such as Open WebUI, to access YAR easily.
...
git clone https://github.com/HKUDS/yar.git
...
### Before Starting YAR Server

YAR necessitates the integration of both an LLM (Large Language Model) and an Embedding Model to effectively execute document indexing and querying operations. Prior to the initial deployment of the YAR server, it is essential to configure the settings for both the LLM and the Embedding Model. YAR supports binding to various LLM/Embedding backends:
...
It is recommended to use environment variables to configure the YAR Server. There is an example environment variable file named `env.example` in the root directory of the project. Please copy this file to the startup directory and rename it to `.env`. After that, you can modify the parameters related to the LLM and Embedding models in the `.env` file. It is important to note that the YAR Server will load the environment variables from `.env` into the system environment variables each time it starts. **YAR Server will prioritize the settings in the system environment variables to .env file**.
...
> **Important Note**: The Embedding model must be determined before document indexing, and the same model must be used during the document query phase. For certain storage solutions (e.g., PostgreSQL), the vector dimension must be defined upon initial table creation. Therefore, when changing embedding models, it is necessary to delete the existing vector-related tables and allow YAR to recreate them with the new dimensions.

### Starting YAR Server

The YAR Server supports two operational modes:
...
```
yar-gunicorn --workers 4
```

When starting YAR, the current working directory must contain the `.env` configuration file. **It is intentionally designed that the `.env` file must be placed in the startup directory**. The purpose of this is to allow users to launch multiple YAR instances simultaneously and configure different `.env` files for different instances. **After modifying the `.env` file, you need to reopen the terminal for the new settings to take effect.** This is because each time YAR Server starts, it loads the environment variables from the `.env` file into the system environment variables, and system environment variables have higher precedence.
...
- `--workspace`: Workspace name, used to logically isolate data between multiple YAR instances (default: empty)

### Launching YAR Server with Docker

Using Docker Compose is the most convenient way to deploy and run the YAR Server.

* Create a project directory.

* Copy the `docker-compose.yml` file from the YAR repository into your project directory.
...
* Start the YAR Server with the following command:
...
There are two ways to start multiple YAR instances. The first way is to configure a completely independent working environment for each instance. This requires creating a separate working directory for each instance and placing a dedicated `.env` configuration file in that directory. The server listening ports in the configuration files of different instances cannot be the same. Then, you can start the service by running `yar-server` in the working directory.

The second way is for all instances to share the same set of `.env` configuration files, and then use command-line arguments to specify different server listening ports and workspaces for each instance. You can start multiple YAR instances in the same working directory with different command-line arguments. For example:

```
# Start instance 1
yar-server --port 9621 --workspace space1

# Start instance 2
yar-server --port 9622 --workspace space2
```
...
- **PostgreSQL stack:** YAR now standardizes on PostgreSQL; workspace prefixes are applied consistently across KV, vector, doc-status, and graph tables (AGE/pgvector).
...
We provide Ollama-compatible interfaces for YAR, aiming to emulate YAR as an Ollama chat model. This allows AI chat frontends supporting Ollama, such as Open WebUI, to access YAR easily.

### Connect Open WebUI to YAR

After starting the yar-server, you can add an Ollama-type connection in the Open WebUI admin panel. And then a model named `yar:latest` will appear in Open WebUI's model management interface. Users can then send queries to YAR through the chat interface. You should install YAR as a service for this use case.
...
The default query mode is `hybrid` if you send a message (query) from the Ollama interface of YAR. You can select query mode by sending a message with a query prefix.

A query prefix in the query string can determine which YAR query mode is used to generate the response for the query. The supported prefixes include:
...
For example, the chat message `/mix What's YAR?` will trigger a mix mode query for YAR. A chat message without a query prefix will trigger a hybrid mode query by default.

`/bypass` is not a YAR query mode; it will tell the API Server to pass the query directly to the underlying LLM, including the chat history. So the user can use the LLM to answer questions based on the chat history. If you are using Open WebUI as a front end, you can just switch the model to a normal LLM instead of using the `/bypass` prefix.

`/context` is also not a YAR query mode; it will tell YAR to return only the context information prepared for the LLM. You can check the context if it's what you want, or process the context by yourself.

### Add user prompt in chat

When using YAR for content queries, avoid combining the search process with unrelated output processing, as this significantly impacts query effectiveness. User prompt is specifically designed to address this issue — it does not participate in the RAG retrieval phase, but rather guides the LLM on how to process the retrieved results after the query is completed. We can append square brackets to the query prefix to provide the LLM with the user prompt:
...
By default, the YAR Server can be accessed without any authentication. We can configure the server with an API Key or account credentials to secure it.
...
The API key is passed using the request header `X-API-Key`. Below is an example of accessing the YAR Server via API:
...
YAR API Server implements JWT-based authentication using the HS256 algorithm. To enable secure access control, the following environment variables are required:
...
RESOURCE_GROUP_NAME=YAR
LOCATION=swedencentral
RESOURCE_NAME=YAR-OpenAI
...
## YAR Server Configuration in Detail
...
YAR supports binding to various LLM/Embedding backends:
...
```
yar-server --llm-binding openai --help
yar-server --llm-binding ollama --help
yar-server --embedding-binding ollama --help
```
...
YAR uses 4 types of storage for different purposes:
...
YAR Server offers various storage implementations, with the default being an in-memory database that persists data to the WORKING_DIR directory. Additionally, YAR supports a wide range of storage solutions including PostgreSQL, MongoDB, FAISS, Milvus, Qdrant, Neo4j, Memgraph, and Redis. For detailed information on supported storage options, please refer to the storage section in the README.md file located in the root directory.
...
When switching the storage implementation in YAR, the LLM cache can be migrated from the existing storage to the new one. Subsequently, when re-uploading files to the new storage, the pre-existing LLM cache will significantly accelerate file processing. For detailed instructions on using the LLM cache migration tool, please refer to[README_MIGRATE_LLM_CACHE.md](../tools/README_MIGRATE_LLM_CACHE.md)

### YAR API Server Command Line Options
...
| --key                 | None          | API key for authentication. Protects the YAR server against unauthorized access                                            |
...
Reranking query-recalled chunks can significantly enhance retrieval quality by re-ordering documents based on an optimized relevance scoring model. YAR currently supports the following rerank providers:
...
```json
{
  "response": "YAR is a graph-based RAG system...",
  "references": [
    {
      "reference_id": "1",
      "file_path": "/documents/intro.md",
      "content": [
        "YAR is a retrieval-augmented generation system that combines knowledge graphs with vector similarity search...",
...
YAR implements asynchronous document indexing to enable frontend monitoring and querying of document processing progress. Upon uploading files or inserting text through designated endpoints, a unique Track ID is returned to facilitate real-time progress monitoring.
