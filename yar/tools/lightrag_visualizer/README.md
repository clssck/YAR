...
An interactive 3D graph visualization tool included in the YAR package for visualizing and analyzing RAG (Retrieval-Augmented Generation) graphs and other graph structures.
...
### Quick Install
```bash
pip install yar-hku[tools]  # Install with visualization tool only
# or
pip install yar-hku[api,tools]  # Install with both API and visualization tools
```
...
1. **Command Not Found**
   ...
   pip install yar-hku[tools]

   # Verify installation
   pip list | grep yar-hku
   ```
...
- GitHub Issues: [YAR Repository](https://github.com/HKUDS/YAR)
- Documentation: [YAR Docs](https://URL-to-docs)

## License

This tool is part of YAR and is distributed under the MIT License. See `LICENSE` for more information.

Note: This visualization tool is an optional component of the YAR package. Install with the [tools] option to access the viewer functionality.
