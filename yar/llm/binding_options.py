...
from yar.constants import DEFAULT_TEMPERATURE
...
# in YAR. It provides a standardized framework for:
#
...
class BindingOptions:
    ...
    def generate_dot_env_sample(cls):
        """
        Generate a sample .env file for all YAR binding options.
        ...
        sample_top = (
            ...
            + '\n'
            + (
                '# Autogenerated .env entries list for YAR binding options\n'
                '#\n'
                '# To generate run:\n'
                '# $ python -m yar.llm.binding_options\n'
            )
            + '#' * 80
            + '\n'
        )

        sample_bottom = ('#\n# End of .env entries for YAR binding options\n') + '#' * 80 + '\n'
    ...
    def options_dict(cls, args: Namespace) -> dict[str, Any]:
        ...
        Example:
            If args contains {'openai_llm_temperature': 0.7, 'other_option': 'value'}
            and this is called on OpenAILLMOptions, it returns {'temperature': 0.7}
        """
...
# 2. If "test" argument is provided, demonstrates argument parsing with OpenAI binding
#
# Usage:
#   python -m yar.llm.binding_options           # Generate .env sample
#   python -m yar.llm.binding_options test      # Test argument parsing
#
# =============================================================================
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'test':
        # Add arguments for OpenAILLMOptions
