...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--input-csv', type=Path, default=DEFAULT_QA_CSV, help='QA CSV with question and expectedResponse columns'
    )
    parser.add_argument(
        '--output-csv',
        type=Path,
        default=None,
        help='Output CSV path (default: yar/evaluation/results/qa_answers_<timestamp>.csv)',
    )
    ...
    parser.add_argument('--mode', type=str, default='hybrid', help='YAR query mode to use')
    parser.add_argument(
        '--response-type', type=str, default='Multiple Paragraphs', help='Response type (default: Multiple Paragraphs)'
    )
    ...
    response = client.post(
        ...
        json={
            ...
            'user_prompt': (
                ...
                'When the question asks about consequences, impacts, or specific items, list ALL of them. '
                'When the context contains a specific template, formula, or syntax pattern, quote it verbatim. '
...
def main() -> None:
    ...
    with httpx.Client(timeout=httpx.Timeout(args.timeout, connect=30.0)) as client:
        for index, test_case in enumerate(test_cases, start=1):
            ...
            try:
                actual_response = query_yar(
                    client, args.api_url, args.api_key, test_case.question, args.mode, args.response_type
                )
