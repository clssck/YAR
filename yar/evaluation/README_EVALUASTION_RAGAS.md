...
**Quick Test:** Index files from `sample_documents/` into YAR, then run the evaluator to reproduce results (~89-100% RAGAS score per question).
...
Results are saved automatically in `yar/evaluation/results/`:
...
```bash
python yar/evaluation/eval_rag_quality.py --dataset path/to/my_dataset.json
```
...
```bash
python yar/evaluation/eval_rag_quality.py -d my_dataset.json -r http://localhost:9621
```

**Absolute path to dataset:**
```bash
python yar/evaluation/eval_rag_quality.py -d /path/to/custom_dataset.json
```
...
EVAL_QUERY_TOP_K=10      # OP_K query parameter of YAR
...
`sample_dataset.json` contains 3 generic questions about YAR. Replace with questions matching YOUR indexed documents.
...
### "YAR query API errors during evaluation"

The evaluation uses your configured LLM (OpenAI by default). Ensure:
- API keys are set in `.env`
- Network connection is stable

### Evaluation requires running YAR API

The evaluator queries a running YAR API server at `http://localhost:9621`. Make sure:
1. YAR API server is running (`python yar/api/yar_server.py`)
2. Documents are indexed in your YAR instance
...
1. Start YAR API server
2. Upload sample documents into YAR  throught  WebUI
3. Run `python yar/evaluation/eval_rag_quality.py`
...
```
INFO: ======================================================================
INFO: 🔍 RAGAS Evaluation - Using Real YAR API
INFO: ======================================================================
...
INFO:   • YAR API:         http://localhost:9621
INFO:   • Results Directory:    results
INFO: ======================================================================
INFO: 🚀 Starting RAGAS Evaluation of YAR System
...
INFO: -------------------------------------------------------------------------------------------------------------------
INFO: 1    | How does YAR solve the hallucination probl... | 1.0000 |  1.0000 | 1.0000 |  1.0000 | 1.0000 |      ✓
INFO: 2    | What are the three main components required in ... | 0.8500 |  0.5790 | 1.0000 |  1.0000 | 0.8573 |      ✓
INFO: 3    | How does YAR's retrieval performance compa... | 0.8056 |  1.0000 | 1.0000 |  1.0000 | 0.9514 |      ✓
INFO: 4    | What vector databases does YAR support and... | 0.8182 |  0.9807 | 1.0000 |  1.0000 | 0.9497 |      ✓
INFO: 5    | What are the four key metrics for evaluating RA... | 1.0000 |  0.7452 | 1.0000 |  1.0000 | 0.9363 |      ✓
INFO: 6    | What are the core benefits of YAR and how ... | 0.9583 |  0.8829 | 1.0000 |  1.0000 | 0.9603 |      ✓
INFO: ===================================================================================================================
INFO:
INFO: ======================================================================
